...
export default class BankcardReport extends React.Component {
  render() {
    const { report, budget } = this.props;
    let result = '';
    if (report.result_desc) {
      result = resultDict[report.result_desc.ANTIFRAUD_INFOQUERY.CreditCardNameIdMobileCheck.card_four_element_consistence];
    }
    return (
      <Form className="detail-form-wrapper">
        ...
        <Row gutter={24}>
          <Col {...col2Props}>
            <FormItem key='cardNumber' {...formItemLayout} label="银行卡号">
              <div className="readonly-text">{budget.cardNumber}</div>
            </FormItem>
          </Col>
          <Col {...col2Props}>
