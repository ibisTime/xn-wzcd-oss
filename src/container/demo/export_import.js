...
class ExportImport extends React.Component {
  ...
  handleExport() {
    ...
    this.createGpXycdydjb(wb);
    this.createGpWtsGsgz(wb);
    this.createGpWtsQtyh(wb);
    this.createXecdydjb(wb);
    this.createGssqwts(wb);
    this.createTjysqwt(wb);
    this.createDbht(wb);
    this.createGpDbht(wb);
    this.createDyht(wb);
    this.createGpDyht(wb);
    // download
  ...
  createGpXycdydjb(wb) {
    // 设置当前页面的的行数，列数，名称
    var ws = this.getSheet(wb, 20, 9, '公牌 新一次抵押登记表（2）');
    // 合并单元格
    ws['!merges'] = [{
      e: {c: 4, r: 5},
      s: {c: 2, r: 5}
  ...
  createGpWtsGsgz(wb) {
    // 设置当前页面的的行数，列数，名称
    var ws = this.getSheet(wb, 17, 5, '公牌 委托书1（公司公章2份）');
    // 合并单元格
    ws['!merges'] = [{
      e: {c: 3, r: 8},
      s: {c: 2, r: 8}
  ...
  createGpWtsQtyh(wb) {
    // 设置当前页面的的行数，列数，名称
    var ws = this.getSheet(wb, 17, 5, '委托书 （其他银行）');
    // 合并单元格
    ws['!merges'] = [{
      e: {c: 3, r: 8},
      s: {c: 2, r: 8}
  ...
  createXecdydjb(wb) {
    // 设置当前页面的的行数，列数，名称
    var ws = this.getSheet(wb, 19, 7, '新二次抵押登记表');
    // 合并单元格
    ws['!merges'] = [{
      e: {c: 3, r: 5},
      s: {c: 2, r: 5}
  ...
  createGssqwts(wb) {
    // 设置当前页面的的行数，列数，名称
    var ws = this.getSheet(wb, 11, 3, '公司授权委托书');
    // 合并单元格
    ws['!merges'] = [{
      e: {c: 7, r: 9},
      s: {c: 6, r: 9}
  ...
  createGpDbht(wb) {
    // 设置当前页面的的行数，列数，名称
    var ws = this.getSheet(wb, 26, 9, '公牌 担保合同');
    // 合并单元格
    ws['!merges'] = [{
      e: {c: 8, r: 3},
      s: {c: 6, r: 3}
  ...
  createDyht(wb) {
    // 设置当前页面的的行数，列数，名称
    var ws = this.getSheet(wb, 31, 10, '抵押合同');
    // 合并单元格
    ws['!merges'] = [{
      e: {c: 2, r: 6},
      s: {c: 1, r: 6}
  ...
  createGpDyht(wb) {
    // 设置当前页面的的行数，列数，名称
    var ws = this.getSheet(wb, 31, 10, '公牌 抵押合同');
    // 合并单元格
    ws['!merges'] = [{
      e: {c: 2, r: 6},
      s: {c: 1, r: 6}
