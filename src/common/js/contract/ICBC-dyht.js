...
export function createHt(data, creditCardTypeList) {
...
function createData(wb, data, creditCardTypeList) {
  ...
  let arr = [
      ...
      ['发动机号', data.engineNo],
      ['贷款（大写）', ''],
      ['贷款（小写）', moneyReplaceComma(moneyFormat(data.loanAmount)) * 1000 / 1000],
      ['履约保证金（大写）', ''],
      ['履约保证金（小写）', moneyReplaceComma(moneyFormat(data.lyAmount))],
      ['年份', year],
      ['月', month],
      ['日', day],
...
function createWtsgh(wb) {
    var ws = getSheet(wb, 22, 8, '委托书 (工行）');
    ws['!merges'] = [{
    e: {c: 3, r: 8},
    s: {c: 2, r: 8}
    }, {
...
// 委托书（其他银行）
function createWtsqtyh(wb) {
    var ws = getSheet(wb, 16, 5, '委托书（其他银行）');
    ws['!merges'] = [{
    e: {c: 3, r: 8},
    s: {c: 2, r: 8}
    }, {
...
// 新二次抵押登记表
function createDecdydjb(wb) {
    var ws = getSheet(wb, 20, 7, '新二次抵押登记表');
    ws['!merges'] = [{
    e: {c: 3, r: 4},
    s: {c: 2, r: 4}
    }, {
...
function createGssqwts(wb) {
    var ws = getSheet(wb, 16, 3, '公司授权委托书');
