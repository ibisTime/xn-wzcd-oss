...
import { moneyFormat, moneyReplaceComma, dateFormat } from 'common/js/util';
export function exportCCBDy(data, creditCardTypeList) {
...
function createData(wb, data, creditCardTypeList) {
  ...
  let arr = [
    ...
    ['履约保证金（大写）', ''],
    ['履约保证金（小写）', moneyReplaceComma(moneyFormat(data.lyAmount)) * 1000 / 1000],
    ['年份', year],
    ['月', month],
    ['日', day],
