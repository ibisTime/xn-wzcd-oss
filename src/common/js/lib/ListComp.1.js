...
import {Form, Select, DatePicker, Input, Button, Table, Cascader} from 'antd';
...
export default class ListComponent extends React.Component {
    ...
    getRealSearchParams(params) {
        ...
        this.options.fields.forEach(v => {
            if (v.type === 'date' || v.type === 'datetime') {
                ...
                let mid = params[v.field].map(a => a === '全部' ? '' : a);
    ...
    getItemByType(type, item) {
        switch (type) {
            ...
            case 'citySelect':
                return this.getCitySelect(item);
            default:
